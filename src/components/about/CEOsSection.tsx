...
export function CEOsSection() {
  ...
  return <section className="bg-black py-20 sm:py-24 md:py-32"><div className="mx-auto max-w-7xl px-6 sm:px-8"><div className="mb-12 sm:mb-16"><p className="editorial-label mb-2 text-white/45">{t.eyebrow}</p><h2 className="font-serif text-4xl font-black leading-[1.1] tracking-tight text-white sm:text-5xl md:text-6xl">{t.title}</h2></div><div className="overflow-hidden rounded-2xl border border-white/15 bg-[#171717] shadow-[0_18px_44px_rgba(0,0,0,0.28)]"><div className="relative flex min-h-[20rem] items-center justify-center overflow-hidden bg-[#0f0f0f] p-4 sm:min-h-[28rem] sm:p-8"><Image src="/images1/imagen-nosotros.png" alt={t.title} fill className="object-contain" sizes="(max-width: 1280px) 100vw, 1152px" /></div><div className="grid grid-cols-1 divide-y divide-white/15 lg:grid-cols-2 lg:divide-x lg:divide-y-0">{t.leaders.map((leader, index) => <article key={leader.name} className="p-6 sm:p-8 lg:p-10"><p className="editorial-label text-white/45">{String(index + 1).padStart(2, "0")} · DRIVEN GROUP</p><h3 className="mt-3 font-serif text-2xl font-semibold tracking-tight text-white sm:text-3xl">{leader.name}</h3><p className="editorial-label mt-2 text-white/55">{leader.role}</p><p className="mt-6 text-base font-light leading-[1.8] text-white/75 sm:text-lg">{leader.bio}</p></article>)}</div></div></div></section>;
}
